...
autoplot.survfit <- function(object, ...,
  ...
  g1 <- ggplot(data=dt1, aes(group=st, colour=st)) +
    geom_step(aes(x=time, y=surv), direction="hv", size=survSize) + theme_bw()
  ###
  type <- match.arg(type)
  if (type=="CI"){
    g1 <- g1 +
  ...
  ### scales
  g1 <- g1 +
    scale_x_continuous(xlab, breaks = times, limits = c(0, max(object$time))) +
  ...
  if(!is.null(assignCol)){
    g1 <- g1 + scale_color_manual(values = assignCol)
  }
  ### remove legend if required
  if(!legend) g1 <- g1 + theme(legend.position = c(leg.x, leg.y))
  ...
  g2 <- ggplot(data=dt3, aes(x=time, y=rev(st), shape=rev(st))) +
    ...
    theme_bw() +
    scale_x_continuous(name="Numbers at risk", limits=c(0, max(object$time)),
                       breaks=times1) +
    ### reverse here to plot in same order as in main plot
    scale_y_discrete(name="", breaks=as.character(levels(dt3$st)),
